...
export class CaixaServiceBusProvider implements LotteryResultsProvider{
 ...
 normalize(lottery:string,raw:unknown){
  ...
  return drawSchema.parse({lottery,contestNumber:Number(x.numero),drawDate:`${date[2]}-${date[1]}-${date[0]}`,numbers:Array.isArray(x.listaDezenas)?x.listaDezenas.map(Number):[],luckyMonth:lottery==="dia-de-sorte"?luckyMonth:undefined,sourceUrl:`${this.base}/${caixaPaths[lottery]}/${Number(x.numero)}`,fetchedAt:new Date().toISOString(),parserVersion:"caixa-servicebus-v1"});
 }
